    ...
    if (_STR_EQ(n->name, "program")) {
        ...
        */
        ...
        if (n->children[1]->children_num == 2) {
            ...
            while (!s.empty()) {
                auto child_node = static_cast<ASTVarDeclaration *>(
                    transform_node_iter(s.top()));
                auto child_node_shared =
                    std::shared_ptr<ASTVarDeclaration>(child_node);
        ...
        if (n->children[2]->children_num == 2) {
            ...
            while (!s.empty()) {
                ...
                auto child_node_shared =
                    std::shared_ptr<ASTStatement>(child_node);
        ...
        */
        ...
        node->if_statement = stmt_node_ptr;

        // check whether this selection statement contains
        // 5.3 else structure

        if (n->children_num == 7) {
            auto stmt_node = static_cast<ASTStatement *>(
                transform_node_iter(n->children[6]));
            auto stmt_node_ptr = std::shared_ptr<ASTStatement>(stmt_node);
            node->else_statement = stmt_node_ptr;
        }
        ...
        if (n->children_num == 3) {
            ...
            */
            ...
            auto additive_expression = static_cast<ASTAdditiveExpression *>(
                transform_node_iter(n->children[0]));
        ...
        // std::cout << "current parent node name:" << n->parent->name <<
        // std::endl; std::cout << "current children num:" << n->children_num <<
        // std::endl;
        std::abort();
    }
    s.push(list_ptr->children[0]);
