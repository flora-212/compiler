...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("add.d", {"$t1, $zero, $zero"});
    ...
    codegen->append_inst("addi.d", {"$t2", "$zero", "0"});
    codegen->append_inst("bcnez", {"$fcc0", "false_label"});
    codegen->append_inst("addi.d", {"$t2", "$t2", "1"});
    codegen->append_inst("false_label:");
    ...
    offset_map["%op2"] = offset_map["%op1"] - 1; // TODO: 请填空
    ...
    codegen->append_inst("ld.d",
                         {"$t0", "$fp", std::to_string(offset_map["%op1"])});
    codegen->append_inst("xor $t1, $t0, $zero");
